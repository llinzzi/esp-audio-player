...
esp_err_t audio_stream_play_url(audio_stream_handle_t h, const char *url) {
    return audio_stream_play_url_with_config(h, url, NULL);
}
...
esp_err_t audio_stream_play_url_with_config(audio_stream_handle_t h, const char *url,
    ...
    audio_stream_t *s = h;
    CHECK_STREAM(s);
    ESP_RETURN_ON_FALSE(url != NULL, ESP_ERR_INVALID_ARG, TAG, "url is NULL");
    ...
    if (cfg) {
        ...
        http_cfg = DEFAULT_AUDIO_HTTP_STREAM_CONFIG(url);
    }
    ...
    audio_stream_io_handle_t io = NULL;
    esp_err_t err = audio_http_stream_get_io(http_stream, &io);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to get stream I/O");
        audio_http_stream_close(http_stream);
        return err;
    }

    err = audio_instance_play_io(s->instance, io);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to play from stream I/O");
        audio_http_stream_close(http_stream);
        return err;
    }
